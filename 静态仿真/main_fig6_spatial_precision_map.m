...
for iFam = 1:numel(families)
    A = OUT.(families{iFam}).anchors;
    st = family_style(families{iFam});
    scatter(A(:,1), A(:,2), 54, ...
        'MarkerFaceColor', st.color, ...
        'MarkerEdgeColor', 'w', ...
        'LineWidth', 0.8, ...
        'DisplayName', sprintf('%s USVs', familyName{iFam}));
end
scatter(0, 0, 115, 'p', ...
    'MarkerFaceColor', [0.85 0.05 0.05], ...
    'MarkerEdgeColor', 'w', ...
...
xlabel('AUV relative x / m');
ylabel('AUV relative y / m');
title('Best formation family regions');
subtitle('White dashed contour marks the target RMSE boundary');
